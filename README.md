...
| `^R` | open a file — **picker**: lists files (`fatls`-style), `↑`/`↓` select, `Enter` open, `Esc` cancel (auto-saves current). In the picker the bottom hint bar switches to file ops (Pine/alpine style): **`n`** new file · **`r`** rename selected · **`d`** delete selected (y/N confirm). Rename is copy-then-delete (this U-Boot has no `fs_rename`); delete/rename are FAT-only (the microSD); new files aren't written until `^S`. Filenames are **ASCII-only** (spaces allowed) — U-Boot's FAT driver can't round-trip Chinese filenames, though Chinese *content* inside files is fine (see [KNOWN_ISSUES.md](KNOWN_ISSUES.md) §3). |
...
| `^\` | toggle **two-column (panel) view** — default off, session-only. The document flows continuously: screen rows fill the **left** panel top-to-bottom, then continue into the **right** panel (like two facing pages). Text soft-wraps to the narrower per-panel width. Moving up only scrolls once the cursor rises above the left panel's first row; moving down walks the left panel, then the right, and only past the last row of the right panel do **both** panels scroll together, one row at a time. |
