...
| `^R` | open a file — **picker**: lists files (`fatls`-style), `↑`/`↓` select, `Enter` open, `Esc` cancel (auto-saves current). In the picker the bottom hint bar switches to file ops (Pine/alpine style): **`n`** new file · **`d`** delete selected (y/N confirm). Delete is FAT-only (the microSD); new files aren't written until `^S`. |
